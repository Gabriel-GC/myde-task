...
function ToastContainer() {
  ...
  return (
    <div
      className="fixed top-3 left-3 right-3 md:top-4 md:right-4 md:left-auto z-50 flex flex-col gap-2.5 md:max-w-sm pointer-events-none"
      role="status"
      aria-live="polite"
    >
      {toasts.map((t) => (
  ...
  return (
    <main className="flex h-screen w-full bg-white overflow-hidden font-sans">
      ...
      <div
        ...
        aria-label="Painel do Chat Selecionado"
      >
        {activeChatId ? (
          <div className="w-full flex flex-col relative">
            <button
              onClick={() => router.push("/")}
              className="md:hidden absolute top-4 left-2.5 z-10 p-2 bg-white rounded-full shadow-md text-neutral-600 border border-neutral-100 flex items-center justify-center cursor-pointer"
